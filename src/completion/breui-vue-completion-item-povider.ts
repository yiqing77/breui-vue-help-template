...
export class breuiCompletionItemProvider implements CompletionItemProvider<CompletionItem> {
 ...
 */
  ...
  getAttrValues(tag: string, attr: string): string[] {
     const config = workspace.getConfiguration().get<ExtensionConfigutation>('breui-vue-helper')
  ...
  provideCompletionItems(document: TextDocument, position: Position, token: CancellationToken, context: CompletionContext): ProviderResult<CompletionItem[] | CompletionList<CompletionItem>> {
    ...
    if (!tag || !/^[B|b]re/.test(tag.text || '')) {
      // 如果不是 breui 的标签(Lay|lay开头) 则返回 null 表示没有hover
      return null
